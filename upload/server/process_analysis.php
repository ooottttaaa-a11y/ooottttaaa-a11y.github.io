...
<head>
    <meta charset='UTF-8'>
    <title>ActivityMonitor - プロセス分析</title>
    ...
    <style>
        ...
        .header-area { background: #fff; padding: 20px; border-bottom: 2px solid #FF9800; margin-bottom: 20px; } /* Orange for Process */
...
$(document).ready(function() {
    table = $('#processTable').DataTable({
        ...
        order: [[1, 'desc']], // Default sort by Total Time desc
        columns: [
            { data: 'process' },
            { 
                data: 'total_time',
...
function loadData() {
    ...
    $.ajax({
        url: `get_process_list.php?start_date=${start}&end_date=${end}`,
        type: 'GET',
        dataType: 'json',
