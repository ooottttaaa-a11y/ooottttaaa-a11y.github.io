...
<head>
    <meta charset='UTF-8'>
    <title>ActivityMonitor - 日次一覧</title>
...
<div class="header-area">
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center">
            <h1 class="h3 m-0">📊 日次一覧: <span id="machineNameTitle"></span></h1>
            <form class="d-flex align-items-center gap-2" onsubmit="event.preventDefault(); loadData();">
                ...
                <a href="index.php" class="btn btn-secondary text-nowrap">一覧に戻る</a>
            </form>
        </div>
    </div>
</div>
...
<div class="container-fluid">
    <div class="card p-4">
        <div class="mb-3" style="display:none;"> <!-- Hidden as we have input fields now -->
            <span class="text-muted">期間: </span><strong id="periodDisplay"></strong>
        </div>
    ...
    table = $('#dailyTable').DataTable({
        ...
        columns: [
            ...
            },
            {
                data: null,
                render: function(data, type, row) {
                    // Start and end date are the same for daily detail
                    return `<a href="detail.php?machine=${encodeURIComponent(machine)}&start_date=${row.date}&end_date=${row.date}" class="btn btn-sm btn-outline-primary">詳細</a>`;
                }
            }
        ]
    });
...
function loadData() {
    ...
    $.ajax({
        url: `get_daily_list.php?machine=${encodeURIComponent(machine)}&start_date=${start}&end_date=${end}`,
        type: 'GET',
        dataType: 'json',
